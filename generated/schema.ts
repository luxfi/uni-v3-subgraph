...
export class Token extends Entity {
  ...
  get token5MinData(): Token5MinDataLoader {
    return new Token5MinDataLoader(
      "Token",
      this.get("id")!.toString(),
      "token5MinData",
    );
...
export class Token5MinData extends Entity {
  constructor(id: string) {
    super();
    this.set("id", Value.fromString(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save Token5MinData entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.STRING,
        `Entities of type Token5MinData must have an ID of type String but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("Token5MinData", id.toString(), this);
    }
  }

  static loadInBlock(id: string): Token5MinData | null {
    return changetype<Token5MinData | null>(
      store.get_in_block("Token5MinData", id),
    );
  }

  static load(id: string): Token5MinData | null {
    return changetype<Token5MinData | null>(store.get("Token5MinData", id));
  }
...
export class Token5MinDataLoader extends Entity {
  _entity: string;
  _field: string;
  _id: string;
  ...
  load(): Token5MinData[] {
    let value = store.loadRelated(this._entity, this._id, this._field);
    return changetype<Token5MinData[]>(value);
  }
}
